...
const Button: React.FC<ButtonProp> = ({
    ...
    if (variant === "secondary") {
        return (
            <button
                className={`${baseClasses} secondary text-[16px] font-[600] px-4 py-2 rounded-md cursor-pointer duration-100 hover:brightness-90`}
                data-content={text}
                style={customStyle}
            >
                {text}
    ...
    return (
        <button
            className={`${baseClasses} ${variantClasses[variant]} text-[16px] font-[600] px-4 py-2 rounded-md cursor-pointer duration-100 hover:brightness-90`}
            style={customStyle}
        >
            {text}
        </button>
